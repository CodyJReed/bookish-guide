...
import { SummarizeInpuSchema, SummarizeOutpuSchema } from "./schemas";

export async function summarize(text: string) {
  const { text: raw } = SummarizeInpuSchema.parse({ text });
  ...
  return SummarizeOutpuSchema.parse({ summary });
}
function clip(raw: string, limit: number) {
